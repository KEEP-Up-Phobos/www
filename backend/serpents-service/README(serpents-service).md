...
This small service exposes `/api/serpents` and attempts to fetch results from a local .NET service at `http://127.0.0.1:5000/api/serpents`. If the .NET service is unavailable it falls back to running the existing `fetcher-cli.js` script from the backend.
...
- Worker:
	- Start the worker with: `node worker.js` (requires Redis at `REDIS_URL` or `redis://127.0.0.1:6379`).
